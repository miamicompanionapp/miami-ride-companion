...
2. **RSS events get a wider net + an honest catch-all.** `daily-refresh.mjs`'s `inferEventCategory()` was rewritten in three layers, checked in order: (a) **`fromDescriptionSegment()`** — many stored events' descriptions still carry the raw Ticketmaster classification text (e.g. `"Sports · Baseball · Starts 18:40 · LoanDepot Park"`), so this is read first as ground truth when present; (b) **title-only keyword rules** for `nightlife`/`arts` triggers (`nightclub`, `theatre`/`theater`, `museum`, `gallery`, …) — these specifically stay title-only because a **venue's own proper name** can false-positive a description match (`"Jackie Gleason Theater"` hosting a concert must not become `arts`; a DJ set's description mentioning `"LIV Nightclub"` shouldn't override the artist's real genre); (c) **title+description keyword rules** for signals that are safe anywhere (`exhibit`, `screening`, `market`/`artisan`, `pilates`/`yoga`/`wellness`, `paws`, `family day`, `cooking class`/`paella`/`byob`, `jazz`, a real `music` rule requiring `concert|tour|live|dj|band|orchestra` etc. — RSS titles are often bare event names like "5LAN in Miami" with the actual "DJ"/"Live" signal only in the blurb). The new **`community`** category (🎪, added to `EVENT_CATS` in `public/index.html` and the manual-add dropdown in `public/editor.html`) is the final fallback — but **only for RSS/manual events**; Ticketmaster is a ticketed concert/sports/arts platform, so an unclassified `tm_` listing (no segment, no keyword — usually a bare artist name like "Mumford & Sons") defaults to `music` instead, since that's overwhelmingly what it is.
3. **Full audit + backfill of `content.json`** (prompted by a follow-up ask to check *all* current events, not just the originally-flagged four) — ran the new logic against the live 62-event set (18 RSS + 44 Ticketmaster) and reclassified 15 more: RSS events that were still `music` (family days, pet meetups, wellness classes, an artisan market, a sunset cruise, a theatre play, an art lecture → mostly `community`, some `arts`) plus two Ticketmaster events the old title-only regex had never caught (`Miami Marlins vs. Seattle Mariners` → `sports`, `Mick Foley: 40 Years of Foley` one-man storytelling show → `comedy`). Caught and fixed two false positives introduced mid-pass before finalizing: an early draft matched `stand.?up` bare (flagged "stand up for the rights of children" as comedy) and bare `children`/`kids` (same false hit) — narrowed to `stand-up\b` and dropped the bare terms.

**Result:** Ticketmaster (44 of 62 current events) gets ground-truth categories straight from the API for new fetches, with a same-shaped fallback for older stored events whose description still has the classification text. RSS events get a much better title+description-based guess, and anything truly ambiguous lands in "Community" instead of silently becoming "Music". Added `tests/backend.spec.js` coverage for `ticketmaster-fetch.js`'s `mapCategory`/`mapEvent` (positive cases per segment, negative case of an unclassified event leaving `category` `undefined`); `daily-refresh.mjs`'s `inferEventCategory()` itself isn't unit-tested — it's a standalone `.mjs` script (ESM, calls `main()` at module scope) that doesn't fit the project's CJS `require()`-based backend test harness, matching its pre-existing (untested) state. Full suite (141 tests) green.

**Files:** `functions/api/ticketmaster-fetch.js`, `scripts/daily-refresh.mjs`, `public/editor.html`, `public/index.html` (`EVENT_CATS`), `public/content.json` (31 events reclassified across both passes), `tests/backend.spec.js`. Bumped `public/sw.js` to **v1.79.0**.
